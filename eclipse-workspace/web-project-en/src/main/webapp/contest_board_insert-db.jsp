...
<%
	...
	int new_id=0;
		try {
			...
			String sql = "select max(id) as max_id, count(*) as cnt from contest";
		...
		%>
		<% 
		sql_update="insert into contest values("+new_id+","+new_id+",'"+multi.getParameter("title")+"','"+session.getAttribute("name")+"','"+session.getAttribute("id")+"','"+multi.getParameter("content")+"','"+file+"','"+time+"', 0,0)";
