...
<body>
	...
	if (totalin.length() > 200 || totalstep.length() > 1000) {
	%>
	<center>
		<h2>길이를 지켜주세요</h2>
		<a href="makerecipe.jsp">뒤로</a>
	</center>
	<%
	} else {
	...
	sql_update = "insert into recipe values(" + new_id + ",'" + name + "','" + content + "','" + totalin + "','" + totalstep
			+ "','" + file + "','" + tag + "','" + myname + "','" + strdate + "'," + view + "," + likes + ")";
	...
	while (files.hasMoreElements()) {
		...
		if (!str.equals("mainimage1")) {
			file = multi.getFilesystemName(str); // 업로드 된 파일 이름 가져옴
			sql_update = "insert into images values(" + imgcnt + "," + new_id + ",'" + file + "')";
			try {
		stmt.executeUpdate(sql_update);
			} catch (Exception e) {
		out.println("DB 연동 오류입니다.:" + e.getMessage());
	...
	stmt.close();
	conn.close();
	out.println("<script>alert('레시피가 작성되었습니다'); location.href='recipepage.jsp';</script>");
	out.flush();
	}
	%>
	<script>
