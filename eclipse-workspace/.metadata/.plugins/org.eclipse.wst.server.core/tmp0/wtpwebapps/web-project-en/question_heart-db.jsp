...
<body>
	<%
		...
		{ 
			...
			sql = "select * from question where id='"+post_id+"'";//-------------------테이블 이름 바꾸기
			rs = stmt.executeQuery(sql);
			...
			new_heart++;
			sql = "update question set heart='"+new_heart+"' where id='"+post_id+"'";
			stmt.executeUpdate(sql);
			
			response.sendRedirect("question_board_post.jsp?id="+post_id);
		...
		else
		{
			...
			<script>
				...
				location.href = "question_board_post.jsp?id=<%=post_id%>";
			</script>
			<% 
		}%>
