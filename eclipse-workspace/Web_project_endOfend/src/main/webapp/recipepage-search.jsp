...
<body>
	...
	String name2 = null;
	String search = "", sel = request.getParameter("sel");
	...
	try {
		...
		if (sel.equals("head")) {
			...
			sql = "select * from recipe where tag like '%" + search + "%'order by likes desc limit " + 8 + " offset "
			+ bpage;
			sql2 = "select * from recipe where tag like '%" + search + "%' order by likes desc limit";
		}
	...
	<div id="wrap">
		<div id="main">
					...
					<tr>
						...
						<td><img src="recipefolder/<%=image%>" class="food"><br>
							...
							<div class="info">
								...
								%>
								<a
									href="dis-like-db.jsp?id=<%=rs.getString("id")%>&likes=<%=rs.getString("likes")%>&search=<%=search%>&sel=<%=sel%>&page=<%=pagenum%>">
									♥ <%=Integer.parseInt(rs.getString("likes"))%></a>
								<%
								} else {
								%>
								<a
									href="add-like-db.jsp?id=<%=rs.getString("id")%>&likes=<%=rs.getString("likes")%>&search=<%=search%>&sel=<%=sel%>&page=<%=pagenum%>">
									♡ <%=Integer.parseInt(rs.getString("likes"))%></a>
								<%
								}
								%>
			...
			<center>
				...
				%>
				<a
					href="recipepage-search.jsp?page=<%=i%>&search=<%=search%>&sel=<%=sel%>"
					class="num selected"> <%
 if (i == pagenum) {
 %><b> <%=i%></b> <%
 } else {
